...
- Calling `$ mkvenv -a <directory>` creates a `.venv` folder inside `<directory>`. `$ mkvenv` then taps `$ source` to run a script named `activate` located in the `.venv` folder that was just created. Running the `activate` script sets `<directory>` as the active Python virtual environment for that shell session. Accordingly, `(<directory>) $` appears to the left of the command line prompt (as shown), signaling that all Python calls made with `<directory>` as the active environment will be routed to the Python interpreter in `/<path>/<directory>/.venv`. To return the Python environment to the system-wide Python install, type `$ deactivate`.
