...
- Calling `$ mkvenv -a <directory>` installs a `.venv` folder inside `<directory>` and then taps `$ source` to run a script named `activate` found in the `.venv` folder that was just created. Running the `activate` script sets `<directory>` as the active Python virtual environment for that shell session. The `activate` script places `(<directory>) $` to the left of the command line prompt (as shown), signaling that all Python calls made while `<directory>` is active will be routed to the Python interpreter in `/<path>/<directory>/.venv`. To return the Python environment to the system-wide Python install, type `$ deactivate`.
