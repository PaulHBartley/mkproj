...
- Calling `$ mkvenv -a <directory>` installs a new folder called `.venv` inside `<directory>`. The `.venv` folder contains the Python resources needed to support `<directory>` as a Python virtual environment. The command then taps `$ source` to run a script named `activate` found in this newly-installed `.venv` folder. Running the `activate` script sets `<directory>` as the active Python virtual environment for that shell session. Following that, the `activate` script places `(<directory>) $` to the left of the command line prompt (as shown), signaling that all Python calls made under the active `<directory>` environment will be routed to the Python interpreter in `/<path>/<directory>/.venv`. To return the Python environment to the system-wide Python install, type `$ deactivate`.
